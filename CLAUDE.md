...
- **Running it in an emulator with the real library:** the bundled APK has no
  ...
  **not a valid frame-rate reading** — only the physical S25 answers invariant 1.5,
  and on that device the target is **8.3ms (120Hz)**, not 16.6ms; see the measured
  numbers under Open items.
...
- **Measured on the physical S25 (SM-S938B), and the frame budget was wrong.**
  The panel is **120Hz**, not 60 — `dumpsys SurfaceFlinger` reports
  `vsyncRate=120.00` throughout play, dropping to 30 only when idle. So the
  budget is **8.3ms**, and the render loop hits it exactly.
  - **Sustained Extreme, unplugged, starting from warm (40°C skin): no derate.**
    Nearly three minutes of continuous play measured 8.3ms p50 / 8.4ms p95 /
    8.4ms p99 and 119.5–120.2 fps, **identical in the first and last rows** —
    thermal status held at 2, skin 40.1 → 40.7°C. Four further minutes of the
    ready screen at status 3 read the same 8.3/8.4. HWUI: p50 7–9ms, p95 11–12ms,
    jank 0.2–1.8%. The panel never dropped its refresh rate.
  - **So the ~46% sustained derate the AAA doc's Wave 2 budget rests on did not
    reproduce**, and that document flags the figure as recalled rather than
    sourced. Do not size a performance budget from it.
  - **But a derate does exist at thermal status 4, and this run never reached
    it.** An earlier session — *plugged in*, so carrying charging heat, and
    longer — reached status 4 at ~45°C skin and showed HWUI p50 climbing 8 → 23ms
    with fps falling to 89. That session had no in-page instrument, so **whether
    the render loop or only HWUI compositing degrades at status 4 is still
    unanswered.** Reaching status 4 is the prerequisite for answering it.
  - **Charging is a confound for any thermal measurement here.** `dumpsys battery`
    → `USB powered: true` adds heat that unplugged play does not, and it is the
    likeliest reason the two sessions landed at different thermal statuses.
  - **The two instruments must share one window, or they will lie to you
    together.** gfxinfo is HWUI compositing the WebView's surface and can read a
    flawless 120fps while the loop inside the page stutters; the in-page rAF probe
    is what the loop actually gets and is blind to whether those frames reach the
    panel. Believing a clean rAF window and a degraded gfxinfo window that never
    overlapped in time is how "no meaningful thermal derate" got said out loud
    here. `scratchpad/both.mjs` samples both against one clock and labels every
    row PLAY / READY / MENU — an unlabelled table that mixes app states looks
    exactly like throttling while being nothing of the kind.
  - **Still unmeasured: the run-start hitch after the warm-up fix.** The probe was
    attached mid-run, so this window cannot see it. One 108.2ms frame appeared at
    31s (4 frames >33ms, 2 >50ms) and is **unattributed** — possibly a song
    transition the 10-second state sampler stepped over.
