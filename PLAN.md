...
- **MD1 (done — with one deliberate deviation)** — Removed the desktop-distribution
  machinery: Docker (`Dockerfile`, `docker-compose.yml`, `.dockerignore`), the
  pinggy tunnel (`scripts/share.sh`), public/read-only mode, `serve:public` and the
  web-serving from Express (the app is bundled into the APK now, so the server
  answers `/api` + `/media` only). Scripts pared to mobile + dev. **Kept the Express
  server as a dev-only backend** rather than deleting it: the browser data layer
  (`web/src/data`, non-native branch) dispatches to it, so it is what makes fast
  browser UI development possible, and it is the desktop content-authoring path
  (`npm run ingest`). Deleting it outright would have meant migrating browser dev
  to the Capacitor web Filesystem (IndexedDB) — a careful change with its own
  `convertFileSrc`/playback questions, deferred rather than rushed. The service
  worker is left as-is: it is browser-only and already skipped on native
  (`main.tsx`), so it is inert in the shipped app.
- **MD2 (done)** — Adaptive launcher icon + splash generated by `@capacitor/assets`
  from `assets/logo.png` (the retro-sun brand mark, same motif as `RetroBackdrop`
  and the play backdrop), on the `#07030f` app background, across every density
  plus dark variants. Verified: the branded icon shows in the launcher. Regenerate
  with `npx capacitor-assets generate --android` after editing `assets/logo.png`.
