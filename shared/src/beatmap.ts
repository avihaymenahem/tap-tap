 ...
 *
 * **Bump this in the same commit that changes note placement.** v0.18.0 did not,
 * and the whole point of the stamp was lost: `fillLongRests` shipped, the
 * desktop library was regenerated by hand, and every device library stayed on
 * the old charts with nothing in the app able to notice. The owner reported it
 * as "Regenerate all doesn't seem to do anything" — the button was fine; the
 * automatic path that should have made pressing it unnecessary never fired.
 *
 ...
 * 4 — `fillLongRests`: no rest longer than `MAX_REST_SEC` while the song still
 *     has onsets to promote (v0.18.0's generation change, stamped late).
 */
